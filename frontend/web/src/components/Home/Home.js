...
function Home() {
  ...
  // Calculate the total number of tasks finished by the user
    ...
    <>
      ...
      {Object.values(activeCourses).map((activeCourse, index) => {
        return (
          <div
            key={index}
            className="bg-white rounded-lg shadow-md p-6 mb-4 border-b pb-4 mt-8"
          >
  ...
  <div className="bg-white text-black p-6 rounded-lg shadow-md mt-4">
    <h2 className="text-xl font-semibold mb-4">Your Activity Breakdown</h2>
    <ul className="list-disc">
      {activities.map((activity, index) => (
        activity.reviwed_by && (
          <li key={index} className="mb-2 space-y-4">
            <span className="font-semibold">
              {activity.domain_name} {activity.course_name} {activity.module_name} {activity.activity_name}:
            </span>
